...
- `.emmyrc.json` — emmylua_check: LuaJIT, strict typeCall + arrayIndex; library paths: `$VIMRUNTIME`, `deps/obsidian.nvim/lua` (clone in CI), `.emmy/`
- `.emmy/lua/vim/` — minimal stubs for Neovim types not covered by `$VIMRUNTIME` (`vim.uv.uv_timer_t`, `vim.SystemObj`)
